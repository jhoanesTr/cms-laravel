                ...
                <div class="flex flex-wrap justify-between">
                    @foreach($posts as $post)
                        <article style="width:300px" class="text-left p-2">
                            <h3 class="py-2 text-xl text-white bg-orange-500 rounded">{{$post->title}}</h3>
                            <p>{{$post->get_limit_body}} <a class="font-bold text-orange-500 no-underline hover:underline" href="{{ route('posts.detail', $post->slug) }}">Read more</a></p>
                        </article>
                    @endforeach
                </div>
            </div>
